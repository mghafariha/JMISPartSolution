...
namespace JMISPartSolution.SetFileItemPermissionFromContract
{
    /// <summary>
    /// List Item Events
    /// </summary>
   public class SetFileItemPermissionFromContract : SPItemEventReceiver
{
    // Methods
    ...
    public override void ItemCheckedIn(SPItemEventProperties properties)
    {
        ...
        try
        {
            SPFileLevel level = properties.ListItem.File.Level;
            base.ItemUpdated(properties);
            string siteURL = properties.Web.Url;
            Guid listId = properties.ListId;
            level = properties.ListItem.File.Level;
                ...
                using (SPSite site = new SPSite(siteURL))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        ...
                        if (list.GetItems(query).Count > 0)
                        {
                            ...
                            int userId = this.GetRelatedUser(web, new SPFieldLookupValue(item1["Creator"].ToString()).LookupId, id, false, item);
                            ...
                            SPFieldLookupValueCollection values3 = (itemById["Viewers"] != null) ? new SPFieldLookupValueCollection(itemById["Viewers"].ToString()) : null;
                            lineNumber = 6;
                            ...
                            lineNumber = 8;
                            SetListItemPermission(item, userId, 0x40000002, true);
                            SetListItemPermission(item, num4, 0x40000002, false);
                            if (num5 != 0)
                            {
                                SetListItemPermission(item, num5, 0x40000002, false);
                            }
                            if (num6 != 0)
                            {
                                SetListItemPermission(item, num6, 0x40000002, false);
                            }
                            SetListItemPermission(item, 0x33, 0x40000002, false);
                            SetListItemPermission(item, 0x34, 0x40000003, false);
                            foreach (int num8 in list5)
                            {
                                SetListItemPermission(item, num8, 0x40000002, false);
                                lineNumber = 9;
                            }
                            foreach (int num9 in list6)
                            {
